...
void Misc::RenderTab()
{
	const char* strafeTypes[] = { "Forwards", "Backwards", "Left", "Right", "Rage", "Sideways (WIP)" };
	const char* animationTypes[] = { "Static", "Marquee", "Words", "Letters", "Loading (WIP)", "Newline (WIP)" };
	...
	ImGui::Columns(2, NULL, true);
	{
		ImGui::BeginChild("Child1", ImVec2(0, 0), true);
		{
			...
			SetTooltip("BunnyHop automatico enquanto segura espaço");
			ImGui::Columns(2, NULL, true);
			{
				ImGui::Checkbox("Edge Jump", &Settings::EdgeJump::enabled);
				SetTooltip("Pula exatamente no final da superficie");
			}
			ImGui::NextColumn();
			{
			...
			ImGui::Columns(2, NULL, true);
			{
				...
				SetTooltip("Strafe automático para bhop");
			}
			ImGui::NextColumn();
			{
			...
			if (Settings::AutoStrafe::type == AutostrafeType::AS_RAGE)
			{
				...
				SetTooltip("Strafes não serão visiveis para espectadores");
			}
			...
			ImGui::Columns(3, NULL, true);
			{
				...
				SetTooltip("Envia uma mensagem no chat quando mata alguem.");
			}
			ImGui::NextColumn();
			{
				ImGui::Checkbox("Team Chat###SAY_TEAM1", &Settings::Spammer::KillSpammer::sayTeam);
				SetTooltip("Manda a Kill Message apenas no chat de equipe");
			}
			ImGui::NextColumn();
			{
			...
			ImGui::NextColumn();
			{
				...
				SetTooltip("Flooda mensagens apenas no chat de equipe");
			}
			ImGui::NextColumn();
			{
			...
			ImGui::Columns(2, NULL, true);
			{
				...
				SetTooltip("Muda o FOV da camera");
				ImGui::Checkbox("Viewmodel FOV", &Settings::FOVChanger::viewmodelEnabled);
				SetTooltip("Muda o FOV do modelo/personagem");
				ImGui::Checkbox("Ignore Scope", &Settings::FOVChanger::ignoreScope);
				SetTooltip("Desativa o Fov Changer quando abrir a mira de awp, etc.");
			}
			ImGui::NextColumn();
			{
				ImGui::PushItemWidth(-1);
			...
			ImGui::Columns(2, NULL, true);
			{
				...
				SetTooltip("Muda a perspectiva de primeira pessoa para terceira pessoa.");
			}
			ImGui::NextColumn();
			{
				ImGui::PushItemWidth(-1);
			...
			ImGui::Columns(2, NULL, true);
			{
				...
				SetTooltip("Mostra lançamentos de granadas salvos, Visuals necessário.");
			}
			ImGui::NextColumn();
			{
				ImGui::Checkbox("Only matching ###match", &Settings::GrenadeHelper::onlyMatchingInfos);
				SetTooltip("Mostra os locais apenas para a granada na sua mão \nNão mostra flashbangs se você está segurando uma smoke.");
			}
	...
	ImGui::NextColumn();
	{
		ImGui::BeginChild("Child2", ImVec2(0, 0), true);
		{
			...
			SetTooltip("Mostra uma clantag customizada");
			ImGui::Separator();
			ImGui::Columns(2, NULL, true);
			{
				ImGui::PushItemWidth(-1);
			...
			if (ImGui::Button("Glitch Name"))
				NameChanger::SetName("\n\xAD\xAD\xAD");
			SetTooltip("Deixa você trocar de nome infinitas vezes, porém não mostra no chat as mudanças de nome.");
			ImGui::SameLine();
			...
			ImGui::Columns(2, NULL, true);
			{
				...
				SetTooltip("Chokes packets (faz você parecer que está lagado).");
				ImGui::Checkbox("Adaptive Fake Lag", &Settings::FakeLag::adaptive);
				SetTooltip("Chokes packets baseado na velocidade (menor valor de choke é o valor do fakelag)");
				ImGui::Checkbox("Auto Accept", &Settings::AutoAccept::enabled);
				SetTooltip("Aceita automaticamente jogos competitivos");
				ImGui::Checkbox("AirStuck", &Settings::Airstuck::enabled);
				SetTooltip("Faz você parar no ar ao pressionar a hotkey (para o tickrate)");
				ImGui::Checkbox("No Key Stuck", &Settings::Airstuck::alwaysOn);
				SetTooltip("Airstuck sem segurar a tecla");
				ImGui::Checkbox("Autoblock", &Settings::Autoblock::enabled);
				SetTooltip("Trava os players automaticamente, impedindo-os de se moverem para frente.\nImita seus movimentos para um melhor bloqueio - bom para trolar");
				ImGui::Checkbox("Jump Throw", &Settings::JumpThrow::enabled);
				SetTooltip("Executa um lançamento de granada pulando perfeitamente. Bom para executar smokes de mapa.");
				ImGui::Checkbox("Auto Defuse", &Settings::AutoDefuse::enabled);
				SetTooltip("Defusa a bomba automaticamente com 0.5 segundos restantes\nCom defuse começa a defusar aos 5.5 segundos restantes até a explosão, e 10.5 segundos sem kit de defuse");
				ImGui::Checkbox("Watermark", &Settings::Watermark::enabled);
				SetTooltip("Ativa a marca d'agua");
				ImGui::Checkbox("Sniper Crosshair", &Settings::SniperCrosshair::enabled);
				SetTooltip("Ativa a crosshair mesmo segurando snipers");
				ImGui::Checkbox("Disable post-processing", &Settings::DisablePostProcessing::enabled);
				SetTooltip("Desativa o post-processing, bom para aumentar o FPS");
			}
			ImGui::NextColumn();
			{
				ImGui::PushItemWidth(-1);
				...
				SetTooltip("Mostra a patente dos outros players no TAB");
				ImGui::Checkbox("Screenshot Cleaner", &Settings::ScreenshotCleaner::enabled);
				SetTooltip("Esconde os hacks visuais ao tirar screenshots");
				...
				SetTooltip("Defusa a bomba sem precisar olhar para ela");
